...
Here is a quick check to count the number of missing data in each column. This is not needed for the data sets in this tutorial that have already been comprehensively examined, but it is helpful to check that the extent of missing values displayed in an R session is what you expect.
...
[^rcbd-1] The Type I method is called “sequential” sum of squares because it adds terms to the model one at a time; while type 3 ("partial") drops model terms one at a time. This can effect the resuls of an F-test and p-value, but only when a data set is unbalanced across treatments, either due to design or missing data points.
