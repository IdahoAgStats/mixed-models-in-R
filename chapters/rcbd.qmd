...
Next, let's load some data. It is located [here](https://here.r-lib.org/) if you want to download it yourself (recommended).
...
tidy_rcbd <- linear_reg() %>% 
  set_engine("lmer", random = ~ 1|block) %>%
  fit(yield_bu_a ~ variety, data = var_trial)
```
