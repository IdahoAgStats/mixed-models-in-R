...
```{r}
dat <- read.csv(here::here("data", "weiss_ICB.csv"))
```
...
hist(dat$yield, main = "", xlab = "yield", cex.lab = 1.8, cex.axis = 1.5, col = base_plot_color)
```

```{r, eval=FALSE}
hist(dat$yield, main = "", xlab = "yield", col = base_plot_color)
```
...
```
```{r, eval=FALSE}
hist(data1$yield, main = "", xlab = "yield", col = base_plot_color)
```
