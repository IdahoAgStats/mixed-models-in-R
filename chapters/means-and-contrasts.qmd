...
`V*N` estimates the marginal means for each combination, and when we do pairwise comparison on these, all combinations are compared to one another simultaneously. 
...
A4 = c(0, 0, 0, 1)
```
These vectors (A1, A2, A3, A4) represent each nitrogen treatment in the order as presented in the m2 emmeans object. A1, A2, A3, and A4 vectors represents 0.0 cwt, 0.2 cwt, 0.4 cwt, and 0.6 cwt nitrogen treatments, respectively. 
...
```
Here the output shows the difference in mean yield between the control and 3 N treatments. The results shows that yield was significantly higher with nitrogen treatments compared to the control (0.0 cwt) irrespective of the oat variety.
...
In addition to these conventional custom contrast options, we can evaluate a few contrasts using the built-in functions in the **emmeans** package. This includes, evaluating the treatments to control or estimating custom contrast to compare one treatment level to all other groups.
...
Let's assume, based on subject matter considerations, if the mean yield of two groups differ by less than 30 it can be considered equivalent. Let's try an equivalence test on the CLDs of nitrogen treatment emmeans (m2).
```{r}
