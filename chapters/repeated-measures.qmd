...
#| column: margin
par(mar=c(5.1, 5, 2.1, 2.1))
hist(phos1$P_leaf, main = "", xlab = "yield", cex.lab = 1.8, cex.axis = 1.5)
```
...
As explained in [**Chapter 6**](factorial-design.qmd), factorial designs involve examining several factors simultaneously. In this section we will add repeated measures component to the factorial design.
...
```{r, eval=FALSE}
corr_str2 = corAR1(form = ~ year1|rep/Ptrt/Inoc/plot, value = 0.2, fixed = FALSE)

fit3 <- lme(P_leaf ~ time*Ptrt*Inoc*Cv,
                random = ~ 1|rep/Ptrt/Inoc/plot,
                corr= corr_str2,
                data = data31, na.action= na.exclude)
tidy(fit3)
...
#| warning: false
#| message: false
check_model(fit3, check = c('normality', 'linearity'))
```
We see a cluster of values in residuals which was due to large number of observations having low values. 
