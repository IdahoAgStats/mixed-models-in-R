...
```{r, eval=FALSE}
hist(sorghum$y, main = "", xlab = "leaf area index", col = base_plot_color)
```
...
hist(sorghum$y, main = "", xlab = "leaf area index", cex.lab = 1.8, cex.axis = 1.5, col = base_plot_color)
```
...
#| column: margin
par(mar=c(5.1, 5, 2.1, 2.1))
hist(alfalfa$yield, main = "", xlab = "yield", cex.lab = 1.8, cex.axis = 1.5, col = base_plot_color)
```

```{r, eval=FALSE}
hist(Yield$Yield, xlab = "yield", col = base_plot_color)
```
### Model fit
...
#| column: margin
par(mar=c(5.1, 5, 2.1, 2.1))
hist(phos1$P_leaf, main = "", xlab = "P leaf", cex.lab = 1.8, cex.axis = 1.5, col = base_plot_color)
```
...
save(pfit1, file = here::here("data/phos_fit1.RData"))
```
