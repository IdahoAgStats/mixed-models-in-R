...
In this example, we have a rice yield data from the [agricolae](https://CRAN.R-project.org/package=agricolae){.external target="_blank"} package (data set "ssp"). The experiment consists of 3 different rice varieties grown under 3 management practices and 5 nitrogen levels in the split-split plot design.
...
The model and analysis of variance of a split-split plot design is divided into three parts: the main plot, split plot and split-split plot analysis.
...
::: column-margin
For information on what `boundary (singular) fit` means, please refer to [**Chapter 14**](troubleshooting.qmd). For now, this warning message will be ignored. 
:::
...
```{r,  fig.height=3}
check_model(model_lmer,  check = c('qq', 'homogeneity', 'reqq'), detrend=FALSE)
```

### nlme

```{r,  fig.height=3}
check_model(model_lme,  check = c('qq', 'homogeneity', 'reqq'), detrend=FALSE)
```
