...
Determining the estimated marginal means for each treatment factor (variety, nitrogen, management) and their interaction.

::: panel-tabset
### lme4
```{r}
emmeans(model_lmer, ~ variety*management)
```
### nlme
```{r}
emmeans(model_lme, ~ variety*management)
```
:::
