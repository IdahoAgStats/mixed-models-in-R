...
In this example, we have a rice yield data from the [agricolae](https://CRAN.R-project.org/package=agricolae) package (data set "ssp"). The experiment consists of 3 different rice varieties grown under 3 management practices and 5 Nitrogen levels in the split-split plot design.
...
```{r, eval=FALSE}
hist(rice$yield, main = "", xlab = "yield")
```
```{r, echo=FALSE}
...
hist(rice$yield, main = "", xlab = "yield", cex.lab = 1.8, cex.axis = 1.5, , col = base_plot_color)
```

### Model Building

The model and analysis of variance of a split-split plot design is divided into three parts: the main-plot, subplot and sub-subplot analysis.
