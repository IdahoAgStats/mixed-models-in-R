...
ggplot(agency_re) +  # need sequence -0.4 to 0.4
  geom_abline(aes(slope = year, intercept = blup_adj, col = `salary slope`), alpha = 0.7, size = 0.75) +
...
Look at Employee Changes in Salary
```{r}
...
```{r}
uw <- filter(wa_salary, agency == "University of Washington", person = "person 30780") 
...
Plot histogram of year slope and year BLUP

```{r, echo=FALSE}
ex_person <- person_re |> arrange(desc(year)) |> slice(1)
