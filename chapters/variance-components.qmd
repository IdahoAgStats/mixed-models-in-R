...
The distibution of the dependent variable, length-to-width ratio,  had a slight left skew, so the the inverse of this trait was modeled. 
...
This is a rather large data set (218,040 rows), and it requires quite a bit of computing power to analyze. Hence, the analysis was done locally, and the data set and model object were saved as a `.RData` file. We wikll load this object into an R session for further exploration. 
...
Unsuprisingly, individual employee is the most impactful factor on salary variance (under the conditions of this analysis).  
...
Note that these 2 syntactical formulations are equivalant: 
...
```{r, echo=FALSE}
#| fig-cap: "Distribution of Single Trial Canola Yield"
#| column: margin
#| echo: false
...
```{r}
#| fig-cap: "Residual vs. fitted plot with unequal vairnace"
