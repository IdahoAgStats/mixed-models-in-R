...
```{r,  fig.height=6}
check_model(oats_lmer, check = c('qq', 'homogeneity', 'reqq'), detrend=FALSE)
check_model(oats_lmer, check = c('qq', 'homogeneity', 'reqq'), detrend=FALSE)
```
...
```{r,  fig.height=6}
check_model(oats_lme, check = c('qq', 'homogeneity', 'reqq'), detrend=FALSE)
```
:::
...
```{r}
car::Anova(oats_lmer, type = "3", test.statistics = "Chisq")
```
