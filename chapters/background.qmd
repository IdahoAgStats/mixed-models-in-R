...
The errors, [For the rest of this tutorial we will be using the term "residual" and not error term to reflect current practices. The residual is calculated as $Y - XB$, it is the gap between the observed value for $Y_i$ and the predicted value, $\hat{Y_i}$]{.aside} $\epsilon_i$, or residual are independently and identically distributed (sometimes called "iid") following a normal distribution for general linear mixed models. Later on we will explore relaxing these assumptions. 
