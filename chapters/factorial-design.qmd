...
hist(data1$yield, main = "", xlab = "yield",cex.lab = cex_lab, cex.axis = cex_axis, col = base_plot_color)
```

```{r, eval=FALSE}
hist(data1$yield, main = NA, xlab = "yield")
```
...
```{r, fig.height=3}
check_model(model1_lmer,  check = c('qq', 'homogeneity', 'reqq'), detrend=FALSE)
```

### nlme

```{r, fig.height=3}
check_model(model2_lme,  check = c('qq', 'homogeneity', 'reqq'), detrend=FALSE)
```
:::
