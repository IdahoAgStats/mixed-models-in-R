...
$$y_{ij} = \mu +  \tau_i+ \beta_j + \tau_i\beta_j + \epsilon_{ij}$$ 
Where: $\mu$ = experiment mean, $\tau$ = effect of factor A, $\beta$ = effect of factor B, and $\tau\beta$ = interaction effect of factor A and B.
...
Last step is to inspect the dependent variable to ensure that there are no extremes (high or low) in the dataset.
...
```{r}
anova(model1_lmer, type = "marginal")
```
### nlme
```{r, fig.height=3}
